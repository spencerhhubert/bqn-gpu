...
function trains, plus statically bounded Repeat, a documented dense Undo
subset, and computational Under over proven inverses. Every claimed valence and domain is listed in
...
The tracked corpus currently contains 368 actual BQN programs and is explicitly designed to grow without a fixed cap. It includes primitive shape cases, phrases, naive/idiomatic pairs, reductions, structural transforms, ordering, major-cell self-search, modifiers, combinators, function trains, bounded iteration, Undo and computational Under, dense mapping/rank algorithms, and a 28-program algorithmic tier spanning statistics, similarity, signal, scan, and matrix workloads. Every case is compiled through the BQN source frontend and compared as a complete value against pinned cBQN. See [docs/corpus.md](docs/corpus.md).
...
cBQN is always a CPU reference in this runner. By default, cBQN arguments and tensor inputs are both constructed before timing, so `resident-compute` rows compare the same synchronous invocation boundary. `--measurement-mode throughput --throughput-batch-size 32` instead warms once, queues a recorded batch, synchronizes once, and reports amortized per-invocation `resident-throughput` timings together with every raw batch duration. The scopes are separate and must never be aggregated together. `--cbqn-timing-scope boundary` separately measures cBQN's current `HostValue` embedding copies and marks those rows `host-value-boundary`; it cannot be combined with throughput mode. The bqn-gpu and native tinygrad implementations use reusable JIT-captured graphs; native Torch uses ordinary eager PyTorch. Every result records language, implementation kind, framework, physical device, timing scope, execution mode, source hash, cold time, raw warm repetitions, batch metadata, and aggregates. Host/device transfer is excluded.
