...
`scripts/run_corpus.py` selects by stable ID glob and tags, scales deterministic inputs, dispatches the compiled source to cBQN, tinygrad, and/or PyTorch, checks correctness, and records cold and repeated warm timings as JSON. Tensor-backend input transfer occurs before timing and output transfer after it. The timed region includes backend dispatch and synchronized execution; cBQN timings include its embedding-boundary value conversion and CPU execution, so consumers should retain the backend labels rather than treating all timing scopes as identical.
